...
template <typename Grammar, basic_fixed_string input, typename ActionSelector = empty_actions, bool IngoreUnknownActions = true> struct parser { // in c++20
	...
	template <size_t Pos, typename Stack = void, typename Subject = void, decision Decision = decision::undecided> struct trampoline_piece;
	
	template <size_t Pos> struct trampoline_placeholder { };
	...
	static constexpr auto move(ctll::accept, Terminal, Stack, Subject) noexcept {
		return trampoline_piece<Pos, Stack, Subject, decision::accept>();
	}
	// if rule is reject => return false and subject
	template <size_t Pos, typename Terminal, typename Stack, typename Subject>
	static constexpr auto move(ctll::reject, Terminal, Stack, Subject) noexcept {
		return trampoline_piece<Pos, Stack, Subject, decision::reject>();
	}
	...
	static constexpr auto move(ctll::pop_input, Terminal, Stack stack, Subject subject) noexcept {
		return trampoline_piece<Pos+1, Stack, Subject, decision::undecided>();
	}
	...
	static constexpr auto move(ctll::list<term<V>, Content...> string, term<V>, Stack stack, Subject subject) noexcept {
		return trampoline_piece<Pos+1, decltype(push_front(list<Content...>(), stack)), Subject, decision::undecided>();
	}
	...
	template <size_t Pos, typename Stack, typename Subject, decision Decision> struct trampoline_piece {
		...
		template <size_t RPos> constexpr auto operator+(trampoline_placeholder<RPos>) const noexcept {
			if constexpr (Decision == decision::reject) {
				return *this;
			} else if constexpr (Decision == decision::accept) {
				return *this;
			} else {
				return decltype(trampoline_piece<RPos, Stack, Subject, Decision>::parse()){};
			}
		}
	};
	...
	template <typename Subject, size_t... Pos> static constexpr auto trampoline_decide(Subject default_subject, std::index_sequence<Pos...>) noexcept {
		...
		return (trampoline_piece<0, decltype(grammar.start_stack), Subject, decision::undecided>::parse() + ... + trampoline_placeholder<Pos+1>());
	}
