...
template <auto V, typename... Ts, typename Parameters> static constexpr auto apply(pcre::create_hexdec, ctll::term<V>, pcre_context_t<ctll::list<Ts...>, Parameters> subject) {
	return pcre_context(ctll::push_front(number<0ull>(), subject.stack), subject.parameters);
}
// hexdec character support (push value)
template <auto V, size_t N, typename... Ts, typename Parameters> static constexpr auto apply(pcre::push_hexdec, ctll::term<V>, pcre_context_t<ctll::list<number<N>, Ts...>, Parameters> subject) {
...
template <auto V, size_t N, typename... Ts, typename Parameters> static constexpr auto apply(pcre::finish_hexdec, ctll::term<V>, pcre_context_t<ctll::list<number<N>, Ts...>, Parameters> subject) {
