...
template <auto V, typename... Ts> constexpr auto operator()(pcre::create_hexdec, ctll::term<V>, ctll::list<Ts...> stack) const {
	return ctll::push_front(number<0ull>(), stack);
}
// hexdec character support (push value)
template <auto V, uint64_t N, typename... Ts> constexpr auto operator()(pcre::push_hexdec, ctll::term<V>, ctll::list<number<N>, Ts...> stack) const {
	constexpr auto previous = N << 4ull;	
	if constexpr (V >= 'a' && V <= 'f') {
		return ctll::push_front(number<(previous + (V - 'a' + 10))>(), ctll::list<Ts...>());
...
template <auto V, uint64_t N, typename... Ts> constexpr auto operator()(pcre::finish_hexdec, ctll::term<V>, ctll::list<number<N>, Ts...> stack) const {
