...
template <typename CurrentStack, char... s> struct ParserCore;
...
template <typename CurrentStack, char f, char... s> struct MoveInput {
	template <bool doActions, typename... Args> static constexpr bool parse(Args && ... args) {
		return ParserCore<CurrentStack, s...>::template parse<doActions>(std::forward<Args>(args)...);
	}
};
...
template <typename CurrentStack, char... s> class ParserCore {
protected:
	...
	template <bool doActions, typename... T, typename... Args> static constexpr bool decide(Stack<T...> move, Args && ... args) {
		return ParserCore<decltype(CurrentStack::Pop::push(move)), s...>::template parse<doActions>(std::forward<Args>(args)...);
	}
...
template <typename CurrentStack, char... s> struct ParserActionDecider<false,CurrentStack,s...> {
	template <bool doActions, typename... Args> static constexpr bool parse(Args && ... args) {
		return ParserCore<CurrentStack,s...>::template decide<doActions>(typename ParserCore<CurrentStack,s...>::Move::N{}, std::forward<Args>(args)...);
	}
};
...
template <typename CurrentStack, char... s> struct ParserActionDecider<true,CurrentStack,s...> {
	template <bool doActions, typename... Args> static constexpr bool parse(Args && ... args) {
		...
		return ParserCore<CurrentStack,s...>::template decide<doActions>(Epsilon{}, std::forward<Args>(args)...);
	}
};

template <typename StartSymbol, char... s> class Parser {
protected:
	using Core = ParserCore<Stack<StartSymbol>,s...>;
public:
	template <typename... Args> static constexpr bool parse(Args && ... args) {
		return Core::template parse<true>(std::forward<Args>(args)...);
	}
	static constexpr bool check() {
		return Core::template parse<false>();
	}
}; 
